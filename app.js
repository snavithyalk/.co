...
function loginSuccess(user) {
    ...
    if (user.isTempPass) {
        document.getElementById('password-change-overlay').classList.remove('hidden');
    }
...
function updateAdminPanel() {
    ...
    users.forEach(u => {
        ...
        const roleSelect = `
            ...
            <select onchange="adminChangeService(${u.id}, this.value)" style="padding:5px; width: 100%; display: ${u.role === 'provider' ? 'block' : 'none'};">
                ...
                <option value="Pedareru" ${u.providerService === 'Pedareru' ? 'selected' : ''}>Pedareru (Masonry)</option>
                <option value="Tile" ${u.providerService === 'Tile' ? 'selected' : ''}>Tile Service</option>
                <option value="Aluminium" ${u.providerService === 'Aluminium' ? 'selected' : ''}>Aluminium Fitting</option>
                <option value="Iron" ${u.providerService === 'Iron' ? 'selected' : ''}>Iron Works</option>
            </select>
        `;
    ...
    requests.slice().reverse().forEach(r => {
        ...
        if (r.status === 'pending_admin') {
            ...
            actionBtns = `<span style="color:green;">Accepted</span>`;
...
window.updateProviderPanel = function () {
    ...
    // Providers only see jobs that are officially ASSIGNED (Approved by Admin)
    const myJobs = requests.filter(r => r.providerId == currentUser.id && r.status !== 'pending_admin' && r.status !== 'rejected');

    if (myJobs.length === 0) {
        list.innerHTML = '<li>No jobs assigned to you yet.</li>';
        return;
    }
    ...
    myJobs.forEach(job => {
        ...
        li.innerHTML = `
            ...
            ${job.status !== 'completed' ? `<button onclick="completeJob('${job.id}')" class="btn-primary mt-2">Mark as Completed</button>` : ''}
        `;
        list.appendChild(li);
    });
};
...
window.loadEzvizDemo = function() {
    const container = document.getElementById('ezviz-container');
    container.innerHTML = `<iframe src="https://open.ezvizlife.com/live/page" style="width:100%; height:100%; border:none;" allowfullscreen></iframe>`;
...
async function renderProfilePage() {
    ...
    if ((currentUser.role === 'provider' && currentUser.plan && currentUser.plan !== 'free') || currentUser.role === 'admin') {
        ...
        document.getElementById('current-company-name').innerText = currentUser.companyName || 'No Company Name Set';
...
function handleProfileUpdate(e) {
    ...
    let updateData = { name, district, nic, email, preferredBranch, salutation, nationality, birthday, gender, sponsor };
    ...
    if (currentUser.role === 'provider') {
        ...
        currentUser.bankBranch = bankBranch;
        currentUser.bankAccount = bankAccount;
        currentUser.bankHolder = bankHolder;
    }
    ...
    db.ref('users/' + currentUser.id).update(updateData).then(() => {
        ...
        currentUser.preferredBranch = preferredBranch;
        currentUser.salutation = salutation;
        localStorage.setItem('navithyaUser', JSON.stringify(currentUser));
        renderProfilePage();
    });
}
